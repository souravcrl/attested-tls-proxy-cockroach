...
### 12. How Measurement Verification Works

**Critical Question:** How does the verifier validate client measurements?

**Answer - Complete Flow:**

**Step 1: Hardware Root of Trust (AMD Secure Processor)**
```
During VM boot, AMD PSP generates launch measurement:

Launch Digest = SHA-384(
    OVMF Firmware (UEFI code),
    Kernel (vmlinuz),
    Initrd (initial ramdisk),
    Command line parameters,
    VMSA (Virtual Machine Save Area - CPU state)
)

→ Stored in AttestationReport.Measurement[48 bytes]
→ Signed by AMD chip's ECDSA P-384 private key
→ Guest OS CANNOT modify or forge this value
```

**Step 2: Attestation Report Generation (Client)**
```go
// Client code (tests/integration/helpers/testclient/attest.go)
func GenerateAttestation(nonce []byte) (*AttestationEvidence, error) {
    // Call AMD Secure Processor via /dev/sev-guest
    fd, _ := syscall.Open("/dev/sev-guest", syscall.O_RDWR, 0)

    // Request includes nonce as report_data
    req := SnpReportReq{
        ReportData: nonce,  // 64-byte user-provided challenge
    }

    // AMD PSP returns 1184-byte signed report
    syscall.Ioctl(fd, SNP_GET_REPORT, &req)

    // Report contains:
    // - Measurement from boot (cannot be faked)
    // - Nonce from request
    // - TCB version
    // - Policy bits (debug, SMT)
    // - Signature (ECDSA P-384)
}
```

**Step 3: Reference Value Distribution (Server-Side)**

**Current Implementation (Phase 2.5) - Local Policy:**
```yaml
# config/attestation-policy.yaml
measurements:
  expected: ""  # Single expected measurement
  allow_list:   # OR multiple acceptable measurements
    - "544553545f4d4541535552454d454e545f56414c49445f30303100..."  # GCP Ubuntu v20250114
    - "def456789abcdef0123456789abcdef0123456789abcdef012345..."  # GCP Ubuntu v20250201
  mode: "strict"  # reject if mismatch
```

**How these reference values are generated:**
```bash
# On a trusted build server with same image
sudo dmesg | grep "SEV-SNP: measurement"
# OR
sudo cat /sys/firmware/efi/efivars/SevLaunchDigest-*
# → Copy SHA-384 hash to policy.yaml
```

**Step 4: Verification (Proxy Server)**
```go
// pkg/policy/verifier.go:208-270
func (v *Verifier) verifyMeasurement(evidence *AttestationEvidence) CheckResult {
    check := CheckResult{
        Name:     "measurement_validation",
        Severity: "critical",  // Fail immediately if wrong
    }

    // Extract actual measurement from report (offset 0x090, 48 bytes)
    actualMeasurement := hex.EncodeToString(evidence.Report.Measurement[:])

    // Compare against expected value from policy
    ...
    // OR check against allow list (multiple acceptable measurements)
...
**Step 5: Signature Verification (Proves Authenticity)**
```go
// pkg/policy/verifier.go (verifySignatureChain)
func (v *Verifier) verifySignature(evidence *AttestationEvidence) CheckResult {
    // Verify AMD certificate chain: Report → VCEK → ASK → ARK
    // 1. Verify ARK is AMD's trusted root (hardcoded)
    // 2. Verify ASK signed by ARK
    // 3. Verify VCEK signed by ASK
    // 4. Verify Report signed by VCEK

    // Extract signature from report (last 512 bytes, ECDSA P-384)
    r := new(big.Int).SetBytes(report.Signature[:48])
    s := new(big.Int).SetBytes(report.Signature[48:96])

    // Hash report bytes (excluding signature field)
    reportBytes := report.Marshal()[:672]  // First 672 bytes only
    hash := sha512.Sum384(reportBytes)

    // Verify with VCEK public key
    if !ecdsa.Verify(vcekPublicKey, hash[:], r, s) {
        return DENY("ECDSA signature invalid - report was tampered with")
    }

    return ALLOW("Signature chain valid")
}
```

**Why This Is Secure:**

1. **Hardware Root of Trust**: Measurement generated by AMD PSP (not guest OS)
2. **Cryptographic Proof**: ECDSA P-384 signature proves authenticity
3. **Zero Trust**: Client cannot forge measurements (would need AMD's private key)
4. **Nonce Binding**: Fresh nonce prevents replay attacks
5. **Complete Chain**: Report → VCEK → ASK → ARK verified

**Attack Scenarios (ALL PREVENTED):**

❌ **Attack 1**: Client tries to fake measurement
```
Client: Changes Report.Measurement to match policy
Result: DENIED - Signature verification fails (hash doesn't match)
```

❌ **Attack 2**: Client reuses old valid attestation
```
Client: Replays previous ALLOWED attestation
Result: DENIED - Nonce validation fails (nonce already consumed)
```

❌ **Attack 3**: Client runs different code but claims correct measurement
```
Client: Boots with modified kernel, claims original measurement
Result: DENIED - AMD PSP generates different measurement, signature won't verify
```

✅ **Valid Client**: Runs correct code, requests fresh nonce
```
1. Requests nonce from /api/v1/nonce
2. Boots with approved image → AMD PSP generates matching measurement
3. Gets attestation report with nonce
4. Verifier checks: measurement matches + nonce valid + signature valid
5. ALLOWED
```
...
Instead of manually including certificates, fetch dynamically:
```go
...
**Phase 3.2 - CoRIM Support:**
Instead of manual YAML, fetch signed manifests from publisher:
```go
corim := fetchManifest("https://mycompany.com/.well-known/corim/atls-proxy-v1.0.corim")
measurements := corim.GetMeasurements()  // Automatically updated on each build
v.policy.Measurements.AllowList = measurements
```

**Phase 3.3 - External Verifier:**
Delegate to attestation service (Veraison, Azure Attestation):
```go
...
// No local policy management needed
```
